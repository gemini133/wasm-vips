...
[ -f "$TARGET/lib/pkgconfig/vips.pc" ] || (
  ...
  meson setup _build --prefix=$TARGET --cross-file=$MESON_CROSS --default-library=static --buildtype=release \
    ...
    -Dresvg-module=enabled -Dspng=enabled ${DISABLE_TIFF:+-Dtiff=disabled} ${DISABLE_WEBP:+-Dwebp=disabled} ${DISABLE_CGIF:+-Dcgif=disabled} -Dnsgif=false -Dppm=false -Danalyze=false \
